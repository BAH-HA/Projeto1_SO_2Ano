...
int tfs_sym_link(char const *target, char const *link_name) {
    ...
    link_inode->i_node_type = target_inode->i_node_type;
...
int tfs_copy_from_external_fs(char const *source_path, char const *dest_path) {
    ...
    char buffer[1024]; // All files are at most one block 
    size_t bytes_read = fread(buffer, 1, 1024, fd);
    if (bytes_read != 1024 && !feof(fd)) { // !feof is true if eof is reached
        return -1;
    }
